...
def create_tokens(output, additional_claims={'group': 'users'}, logger=None):
    token = create_access_token(identity=output, additional_claims=additional_claims, fresh=True)
    refresh_token = create_refresh_token(identity=output)
...
def configure_jwt(app):
    ...
    def my_expired_token_callback(identity,message):
        resp = jsonify({
            'status': 401,
            'sub_status': 42,
